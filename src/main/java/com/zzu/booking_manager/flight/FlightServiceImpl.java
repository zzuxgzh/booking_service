...
public class FlightServiceImpl implements IFlightService {
    ...
    public String changeFlightStatus(Flight flight, String reason) {
        ...
        if(a==0){//错误
            return "您的操作有误，请重试！！";
        }else if(a==1){//记得校验当前航班状态和飞机状态 起飞，修改飞机的状态和航班的状态 发布公告
        ...
        }else if(a==2){//记得校验当前航班状态和飞机状态 到达，修改飞机的状态和location和航班状态
            if(ifd.changeFlightStatusById(flight)==1){
                Plane plane = new Plane();
                plane.setStatus(0);
                plane.setPlaneId(sflight.getPlaneId());
        ...
        }else if(a==4){//记得校验当前航班状态和飞机状态 取消，修改飞机状态为未安排，修改航班状态，发布公告
